...
Matches random users based on gender preference and shared city (via WebApp GPS).
"""
...
async def resume_onboarding(bot, chat_id, step):
    """Re-prompt whichever onboarding step the user is stuck on."""
    await bot.send_message(
        chat_id=chat_id,
        text="⚠️ Please complete your profile setup first."
    )
    if step == STEP_GENDER:
...
async def require_onboarding(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Returns True if user is fully onboarded. Otherwise re-prompts and returns False."""
...
async def exchange_username(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if reqs.get(partner_id) == user_id:
        del reqs[partner_id]
        u = get_user(user_id); p = get_user(partner_id)
...
# ── TEXT HANDLER (menu buttons + relay) ──────────────────────────────────────
